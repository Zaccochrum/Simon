...
class SimonActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private var simonSoundID = IntArray(4)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        simonImageView[0]!!.setCelType(SimonCellType.CELL_TYPE_GREEN)
        simonImageView[1]!!.setCelType(SimonCellType.CELL_TYPE_RED)
        simonImageView[2]!!.setCelType(SimonCellType.CELL_TYPE_YELLOW)
        simonImageView[3]!!.setCelType(SimonCellType.CELL_TYPE_BLUE)
    ...
    private fun litCellsTwo(litCellsIndex: Int) {
        ...
        if (simonPreySoundStreamID != 0) {
            soundPool!!.stop(simonPreySoundStreamID)
        }
        if (simonSoundOn) {
            simonPreySoundStreamID = soundPool!!.play(simonSoundID[litCellsIndex],
    ...
    private fun simonCellClicked (cellNumber: Int, cell: SimonCellType) {
        ...
        if (simonSoundOn) {
            simonPreySoundStreamID = soundPool!!.play(simonSoundID[cellNumber],
            simonSoundVolume, simonSoundVolume,1,0,1f)
        }
        startTimer(cell)
